<?php
require __DIR__.'/inc/cleanup.php';
require __DIR__.'/inc/function-admin.php';
require __DIR__.'/inc/enqueue.php';
require __DIR__.'/inc/theme-support.php';
require __DIR__.'/inc/custom-post-type.php';
require __DIR__.'/inc/generic-hooks.php';
require __DIR__.'/inc/generic-functions.php';
require __DIR__.'/inc/sunset-ajax.php';
require __DIR__.'/inc/shortcode.php';
require __DIR__.'/inc/sunset-widgets.php';
require __DIR__.'/inc/credentials.php';
require __DIR__.'/inc/customize/customize.php';


require __DIR__.'\class\class-wp-bootstrap-navwalker.php';
// require __DIR__.'\class\phpmailer\PHPMailer.php';
// require __DIR__.'\class\phpmailer\SMTP.php';
// require __DIR__.'\class\phpmailer\Exception.php';

?>
